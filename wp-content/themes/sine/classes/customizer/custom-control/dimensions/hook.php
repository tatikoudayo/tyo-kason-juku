...
if( !class_exists( 'Sine_Customizer_Dimensions_Integration' ) ){

	class Sine_Customizer_Dimensions_Integration extends Sine_Customizer{
		...
		public static $type = 'sine-dimensions';

		public static $class = 'Sine_Customizer_Dimensions_Control';
		...
		public function start( $instance , $wp_customize ){
			...
			foreach( $instance::$fields[ self::$type ] as $field ){
				...
				if( !isset( $field[ 'dimension' ] ) ){
					$err = esc_html__( 'Missing key "dimension" passed for Dimension Control.', 'sine' );
	...
	new Sine_Customizer_Dimensions_Integration();
}
