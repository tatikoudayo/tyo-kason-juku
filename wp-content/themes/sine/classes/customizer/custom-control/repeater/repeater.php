...
if ( class_exists( 'WP_Customize_Control' ) ) {
	class Sine_Page_Repeater extends WP_Customize_Control {
		...
		public function to_json() {
			...
			// file_put_contents( __DIR__.'/www.txt' , print_r( $this->json, 1 ) );
		}
			...
			<div class="page-repeater-template hidden">
				<div>
					...
					<button data-index="{#index}" data-limit="{{ data.limit }}" class="page-repeater-remove"> 
						<span class="dashicons dashicons-trash"></span><?php esc_html_e( 'Delete', 'sine' ) ?>
					</button>
				</div>
			</div>
...
Sine_Customizer::add_custom_control( array(
    'type'     => 'sine-repeater',
    'class'    => 'Sine_Page_Repeater',
    'sanitize' =>  array( 'Sine_Page_Repeater', 'sanitize' )
));

function sine_page_repeater_control( $args, $section, $field ){

	if( isset( $field[ 'type' ] ) && 'sine-repeater' == $field[ 'type' ] ){
...
add_filter( 'sine_customizer_get_control_arg', 'sine_page_repeater_control', 10, 3 );

function sine_page_repeater_get( $value, $id, $instance ){
...
add_filter( 'sine_customizer_get', 'sine_page_repeater_get', 10, 3 );