	...
	class Sine_Helper{
		...
		public static function skip_content(){
			...
			<a class="skip-link screen-reader-text" href="#content">
				<?php esc_html_e( 'Skip to content', 'sine' ); ?>
			</a>
			<?php
		}
		...
		*/
		...
		public static function get_comment_number(){
			if ( ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
				echo '<span class="sine-comments">';
				comments_popup_link(
					'<i class="fa fa-comment"></i> '.esc_html__( 'Leave a comment', 'sine' ),		
					'<i class="fa fa-comment"></i> '.esc_html__( '1 response', 'sine' ),
					'<i class="fa fa-comments"></i> % '. esc_html__( 'responses' , 'sine' )
				);
				echo '</span>';
			}
		}		
		...
		*/
		public static function get_assets_version(){
			return sine_get( 'assets-version' );
		}
		...
		*/
		public static function posts_navigation( $echo = true ) {
			...
			if( !$infinity_module_active || !Sine_Helper::is_active_plugin( 'jetpack' ) ){				
				the_posts_pagination(
					array(
						'mid_size'  => 2,
						'prev_text' => esc_html__( 'Previous', 'sine' ),
						'next_text' => esc_html__( 'Next', 'sine' ),
					)
				);
			}
		}
		...
		*/
		public static function post_content_navigation(){
			wp_link_pages( array(
				'before'      => '<div class="page-links">' . esc_html__( 'Pages:', 'sine' ),
                'after'       => '</div>',
		...
		*/
		public static function single_post_navigation(){
			the_post_navigation( array(
				'prev_text' => '<span class="screen-reader-text">' . esc_html__( 'Previous Post', 'sine' ) . '</span><span class="nav-title">%title</span>',
				'next_text' => '<span class="screen-reader-text">' . esc_html__( 'Next Post', 'sine' ) . '</span><span class="nav-title">%title</span>',
			));
		}
		...
		*/
		...
		public static function the_date( $status = 'posted' ) {
			...
			if( $show ):
				...
				printf(
					...
					( 'posted' == $status ) ? esc_html__( 'On', 'sine' ) : esc_html__( 'Updated on', 'sine' ),
					$time_tag
				);
			endif;
		}
		...
		public static function the_breadcrumb(){
			...
			if( apply_filters( self::fn_prefix( 'show_breadcrumb' ), true ) ): 			    
			    ...
			    <div id="<?php echo esc_attr( self::with_prefix( 'breadcrumb' ) ) ?>" class="<?php echo esc_attr( $classes ); ?>">
			    	<?php sine_breadcrumb_trail( $breadcrumb_args ); ?>
				</div><!-- #breadcrumb -->
			    <?php return;
			endif;
		}
					...
					<?php 
						...
						<?php }elseif(  is_search() ){
							...
							<h2 class="entry-title">
								<?php 
									printf( 
										esc_html__( 'Search Results for: %s', 'sine' ), 
		...
		/**
		* Font family used in Sine Theme
		*
		* @static
		* @access public
		* @return object
		...
		*/
		public static function get_font_family( ){
			return apply_filters( self::fn_prefix( 'standard_fonts_array' ), array(
				'font-1'  => esc_html__( 'Lato', 'sine' ),
				'font-2'  => esc_html__( 'Oswald', 'sine' ),
				'font-3'  => esc_html__( 'Montserrat', 'sine' ),
				'font-4'  => esc_html__( 'Roboto', 'sine' ),  
				'font-5'  => esc_html__( 'Raleway', 'sine' ),  
				'font-6'  => esc_html__( 'Playfair Display', 'sine' ),  
				'font-7'  => esc_html__( 'Fjalla One', 'sine' ),  
				'font-8'  => esc_html__( 'Alegreya Sans', 'sine' ),
				'font-9'  => esc_html__( 'PT Sans Narrow', 'sine' ),
				'font-10' => esc_html__( 'Open Sans', 'sine' ),
				'font-11' => esc_html__( 'Poppins', 'sine' ),
			));
		}
		...
		public static function get_google_font(){
			...
			$google_fonts[] = self::uglify( self::get_font( sine_get( 'body-font' ) ), $condition );
			$google_fonts[] = self::uglify( self::get_font( sine_get( 'heading-font' ) ), $condition );
			$google_fonts[] = self::uglify( self::get_font( sine_get( 'site-info-font' ) ), $condition );
